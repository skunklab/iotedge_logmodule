...
namespace SkunkLab.VirtualRtu.Server
{
    public class Listener
    {
        ...
        {
            //IChannel channel = new TcpServerChannel2(client, rtuConfig.MaxBufferSize, CancellationToken.None);
            src = new CancellationTokenSource();
            IChannel channel = ChannelFactory.Create(false, client, rtuConfig.BlockSize, rtuConfig.MaxBufferSize, src.Token);
                //new TcpServerChannel2(client, 1024, rtuConfig.MaxBufferSize, src.Token);
            string id = channel.Id;
            VirtualRtuAdapter adapter = null;
            try
            {
                adapter = new VirtualRtuAdapter(rtuConfig, map, channel);
                //adapter = new VirtualRtuAdapter(rtuConfig, map, mqttConfig, channel);
